...
export const jobSchema = validate.object({
	...
	salary_max: validate.number().min(0, "Salary Max must be 0 or greater").optional().nullable(),	
	job_description: validate.string().min(1, "Job Description is required"),
	job_experience_min: validate.number().min(0, "Job Experience Min must be 0 or greater"),
	job_experience_max: validate.number().min(0, "Job Experience Max must be 0 or greater").optional().nullable(),
